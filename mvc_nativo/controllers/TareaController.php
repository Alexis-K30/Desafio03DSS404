...
class TareaController {
    ...
    public function cambiarEstado(): void {
        ...
        // === NUEVO: Calcular si está vencida o por vencer ===
        $esVencida   = false;
        $esPorVencer = false;
        ...
        if ($ok) {
            ...
            if ($tarea && !empty($tarea['fecha_limite'])) {
                $fechaLimite = $tarea['fecha_limite'];

                $esVencida = ($fechaLimite < $hoy) 
                             && !in_array($estado, ['completada', 'cancelada']);

                $esPorVencer = ($fechaLimite === $hoy) 
                               && !in_array($estado, ['completada', 'cancelada']);
            }
        }
